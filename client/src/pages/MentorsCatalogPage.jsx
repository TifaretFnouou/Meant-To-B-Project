...
//   const { createRequest, Meetings } = useScheduling();
...
export default function MentorsCatalogPage() {
  ...
  const { Meetings = [], refreshMeetings } = useScheduling();
  ...
  // a smart state: lock the button immediately when the user clicks on it,
  // so she sees that the request "is waiting" even before the server returns all the Meetings again.
  ...
  const hasPendingWith = (mentorId) => {
    // 1. quick check if we clicked on this mentor
    if (localPendingRequests.includes(mentorId)) return true;

    // 2. check against the real database that comes from the context
    return Meetings.some((s) => {
      // extract the id even if the server returned it as an ObjectId (Populated)
      const sMentorId = typeof s.mentorId === "object" ? s.mentorId?._id : s.mentorId;
      const sMenteeId = typeof s.menteeId === "object" ? s.menteeId?._id : s.menteeId;

      const isMatch = String(sMentorId) === String(mentorId) && String(sMenteeId) === String(currentUser?.id);

      // identify active statuses only (not cancelled and not completed)
      const status = String(s.status).toLowerCase();
      const schedulingState = String(s.schedulingState).toLowerCase();

      const isActive =
        !["cancelled", "rejected", "completed"].includes(status) &&
        !["cancelled", "rejected", "completed"].includes(schedulingState);
  ...
  // the function is called by the MentorCard after the API call has been successful
  const handleInterest = async (mentor) => {
    if (!isMenteeMode) return;
    
    //  lock the button specifically for this mentor
    setLocalPendingRequests((prev) => [...prev, mentor.id]);

    // optional: refresh the Meetings list from the server if the context supports it
